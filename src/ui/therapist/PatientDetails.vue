      ...
      <md-card-content>
        <div class="container">
          <div class="form">
            ...
            <div>
                <div class="md-subhead">Difficulty Level in WristAngle Classifier</div>
                <md-radio v-model="currentPatient.enabled_gesture" @change="classifierChange" value="wrist-angle">Wrist Angle</md-radio>
                <md-radio v-model="currentPatient.enabled_gesture" @change="classifierChange" value="thumb-index">Thumb Index</md-radio>            </div>
          </div>
        </div>
      </md-card-content>
          ...
          if (response.data.success) {
            ...
            if (response.data.graphs && response.data.graphs.length > 0) {
              ...
              for (var i = 0; i < graphsArr.length; i++) {
                count++;
                if (graphsArr[i].classifier_name == "thumb-index") {
                  graphsArr[i].identifier = "TI-LEAP";
                } else {
                  graphsArr[i].identifier = "WA-LEAP";
                }
              }
              if (count == 0) {
